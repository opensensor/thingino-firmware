...
BOOTLOADER=${BR2_TARGET_UBOOT_BOARD_DEFCONFIG:-${BR2_TARGET_UBOOT_BOARDNAME:-}}
if [ -z "$BOOTLOADER" ]; then
	BOOTLOADER=$(sed -nE 's/^BR2_TARGET_UBOOT_(BOARD_DEFCONFIG|BOARDNAME)="([^"]+)"$/\2/p' "$BR2_CONFIG" | head -1)
fi
